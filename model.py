...
# 	x = layers.Input(shape=(1, 512, 512, 4))
...
def IDCN2(w, h, nFilters, nPyramids, Pyramid_Cells, nPyramidFilters, qy, qc, Restriction='hard'):
	output_list = []
	def dual_domain_block(x, nFilters, Pyramid_Cells, nPyramidFilters, qy, qc, Restriction):
		_t = pyramid(x, nPyramidFilters, Pyramid_Cells)
...
# 	print('flops: ', flops.total_float_ops / 1e6)
...
#   # model_head = os.path.join(model_path, 'IDCN_' + str(qf) + '_8_64_best.h5')
#   # PyramidCells = ((3, 2, 1, 1, 1, 1),)
#   # model = IDCN(64, 8, PyramidCells, 64, qy, qc)
#   # # model.load_weights(model_head)
#   # img = np.concatenate([img, sigma[0:img.shape[0], 0:img.shape[1], :]], axis=-1)
#   # img = img.reshape((1,) + img.shape)
#   # # img_db = model.predict(img)
